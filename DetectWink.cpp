...
string FACES_CASCADE_NAME_ORIGINAL = cascades + "haarcascade_frontalface_alt2.xml";
...
int detect(Mat frame,
	...
	*/
	...
	//Face does not detected. Go for another xml file.
	if (nfaces == 0) {
		CascadeClassifier alternate;
		alternate.load(FACES_CASCADE_NAME_ORIGINAL);
		cascade_face.detectMultiScale(frame_gray, faces,
	...
	for (int i = 0; i < nfaces; i++) {
		...
		float y1 = face.y + face.height/6.0;

		Rect lian = Rect(face.x, y1, face.width, face.height / 2.0);
		
		Mat faceROI = frame_gray(lian);
		if (detectWink(frame, Point(face.x, y1), faceROI, cascade_eyes)) {
